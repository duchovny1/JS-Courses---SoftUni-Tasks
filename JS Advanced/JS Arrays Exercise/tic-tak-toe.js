function solve(input){
    let dashboard = [[false, false, false],
    [false, false, false],
    [false, false, false]];

    const firstPlayerMark = 'X';
    const secondPlayerMark = 'O';
    
    let activePlayer = firstPlayerMark;
    let freeAreas = 9;
    let winner = '';
    

     for(let line of input){
      let[row, column] = line.split(' ').map(x => Number(x));

      dashboard[row][column] === false ? markPlace(row,column) : chooseOtherPlace();

      if(winner !== ''){
          break;
      }

      if(freeAreas === 0){
        console.log("The game ended! Nobody wins :(")
        break;
       } 
     
    }
    
    return dashboard.map((n) => n.join("\t")).join("\n")

    function markPlace(row, column){
        dashboard[row][column] = activePlayer;
        checkIfWinner(row, column);
        freeAreas--;
        activePlayer === firstPlayerMark ? activePlayer = secondPlayerMark 
           : activePlayer = firstPlayerMark

    }
     
    function checkIfWinner(row, column){
        checkForLine(...dashboard[row]);
        checkForLine(dashboard[0][column], dashboard[1][column], dashboard[2][column]);
        checkForLine(dashboard[0][0], dashboard[1][1], dashboard[2][2]);
        checkForLine(dashboard[0][2], dashboard[1][1], dashboard[2][0]);

        function checkForLine(a, b, c){
            if(a === b && a === c && a !== false){
                winner = activePlayer;
                console.log(`"Player ${winner} wins"`)
                return winner;
            }
        }
    }

    function chooseOtherPlace(){
          console.log("This place is already taken. Please choose another!");
    }
    


}


console.log(solve(["0 1",
"0 0",
"0 2", 
"2 0",
"1 0",
"1 1",
"1 2",
"2 2",
"2 1",
"0 0"]));